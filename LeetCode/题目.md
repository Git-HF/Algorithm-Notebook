...
> * 对于这种存在括号嵌套的，典型的用栈来做，当遇到`']'`时，出栈，解括号。由于题目已经说了确保表达式的正确性，所以不用进行表达式是否合理的判断。时间复杂度为**O(N)**，空间复杂度为栈的空间**O(N)**。[我的代码](Source/decodeString.h)。[将字符串栈和数字栈分开](Source/decodeString_1.h)。

...
> * 当使用递归反转链表时，递归函数返回已经返回已经反转过的链表的尾节点，然后重新连接一下当前结点和尾节点就好。为了最后返回反转的链表，需要定义一个全局变量，保存反转链表后的头节点。时间和空间复杂度均为**O(N)**。[我的代码](Source/reverseList.h)；[别人的代码](Source/reverseList.java)**（必看）**。
...
> * 根据题目，可以使用回溯进行分割字符串。注意细节。[我的代码](Source/restoreIpAddresses.h)。

## 41. 缺失的第一个正数
> * [题目链接](https://leetcode-cn.com/problems/first-missing-positive/solution/)
> * 根据题意，时间复杂度要求**O(N)**，空间复杂度要求**O(1)**。必定要修改原数组，要么是做标记，要么是交换元素。由于该题中元素没有限制，可以是任意整数，所以无法做特殊的标记。**注意边界条件的处理**。[我的代码](Source/firstMissingPositive.h)。[改进版代码，减少了异常情况的处理](Source/firstMissingPositive_1.h)
> * [官方解答的思想，好好看看](https://leetcode-cn.com/problems/first-missing-positive/solution/que-shi-de-di-yi-ge-zheng-shu-by-leetcode-solution/)

## 958. 二叉树的完全性检验
> * [题目链接](https://github.com/afatcoder/LeetcodeTop/blob/master/bytedance/backend.md)
> * 层序遍历。当第一次遍历到NULL时，做一个记录。如果在遇到NULL以后，还遇到非空节点，说明这不是完全二叉树。[我的代码](Source/isCompleteTree.h)。[改进版代码](Source/isCompleteTree_1.h)

## 34. 在排序数组中查找元素的第一个和最后一个位置
> * [题目链接](https://leetcode-cn.com/problems/find-first-and-last-position-of-element-in-sorted-array/)
> * 主要训练二分查找，查找target元素的左边界和右边界。[我的代码](Source/searchRange.h)。注意与查找第一个比target小或者第一个比target大的元素的区别，一个等于时要分开写，一个不用分开写。

## 4. 寻找两个正序数组的中位数
> * [题目链接](https://leetcode-cn.com/problems/median-of-two-sorted-arrays/)
> * 直接将两个数组合并成一个数组，返回n/2位置的元素即可。
> * 直接在原数组查找，查找第n/2+1大的数。查找n/2+1而不是n/2的目的是为了数组总长度为偶数时方便。[我的代码](Source/findMedianSortedArrays.h)。[别人的代码](Source/findMedianSortedArrays_1.h)
> * **二分查找**。（代码未实现）

## 268. 缺失数字
> * [题目链接](https://leetcode-cn.com/problems/missing-number/)
> * **正负标记位**：根据题意，要想实现线性时间复杂度和常数空间，明显使用标志位方法。由于数组中包含0元素，所以不能直接使用正负的标志位。先将数组所有元素+1，这样就可以使用正负标记位了。[我的代码](Source/missingNumber.h)
> * **对应位置标志位**：[我的代码](Source/missingNumber_1.h)
> * **哈希表**：先将所有元素加入哈希集合中，然后判断0-n中的数字是否被包含在其中。空间复杂度为**O(N)**。
> * **异或运算**：任何一个数和自己异或都是0。所以将数组中的N个数和0-N中的每个数异或，最终结果就是缺失的那个数。[我的代码](Source/missingNumber_2.h)
> * **数学求和**：将所有数加起来，然后用0-N的和减去这个数，结果就是缺失的数。(求和时小心和溢出)

## 19. 删除链表的倒数第N个节点
> * [题目连接](https://leetcode-cn.com/problems/remove-nth-node-from-end-of-list/)
> * **一趟遍历**：[我的代码](Source/removeNthFromEnd.h)。注意边界条件（删除第一个元素）。[官方代码](Source/removeNthFromEnd_1.h)

## 69. x的平方根
> * [题目链接](https://leetcode-cn.com/problems/sqrtx/)
> * **二分查找**：[我的代码](Source/mySqrt.h)。注意二分查找的模板

## 449. 序列化和反序列化二叉搜索树
> * [题目链接](https://leetcode-cn.com/problems/serialize-and-deserialize-bst/)
> * **不管搜索树这个条件，直接序列化**：把搜索树当作普通的二叉树进行序列化，只需要正确记录每个节点值和空节点即可。[我的代码](Source/serialize_BST.h)
> * 由于题目限制了是二叉搜索树，所以可以不用记录空节点可以利用元素大小关系就可以反序列化，可以节省空间，但是更加麻烦；或者将所有元素排序，即可得到中序遍历序列。根据中序和前序（或者后续）进行反序列化。**官方解答给了很多优化的地方**。[官方解答](https://leetcode-cn.com/problems/serialize-and-deserialize-bst/solution/xu-lie-hua-he-fan-xu-lie-hua-er-cha-sou-suo-shu-2/)

## 1143. 最长公共子序列
> * [题目连接](https://leetcode-cn.com/problems/longest-common-subsequence/)
> * **经典动态规划**：注意代码的编写，将dp数组创建成M+1和N+1，可以不用额外初始化。如果需要初始化，也得遵循dp方程的形式。[我的代码](Source/longestCommonSubsequence.h)

## 718. 最长重复子数组
> * [题目链接](https://leetcode-cn.com/problems/maximum-length-of-repeated-subarray/)
> * **经典动态规划**：[我的代码](Source/findLength.h)。该问题实际上就是最长公共子串问题。注意和最长公共子序列的区别。

## 300. 最长上升子序列
> * [题目链接](https://leetcode-cn.com/problems/longest-increasing-subsequence/)
> * **暴力版动态规划**：对于每一个数，从前面查找比他小的数，然后记录最大的dp数组值。然后自己等于该值+1。cur_max初始化为0，这样后面可以统一处理。时间复杂度为**O(N^2)**。[我的代码](Source/lengthOfLIS.h)

## 662. 二叉树最大宽度
> * [题目链接](https://leetcode-cn.com/problems/maximum-width-of-binary-tree/)
> * **错误版本**：层序遍历整个树，然后计算每一层节点个数，记录最大的节点个数。**当上一层有空节点时则无法正确计算出该层的左右边界**。[错误代码](Source/widthOfBinaryTree.h)
> * **BFS**：对于每个节点，记录它在完全二叉树中的位置，左节点为 **2 x i+1** ， 右节点为**2*i+2** ，注意数据溢出的情形，主要是每个节点的位置，可能会整数溢出。[我的代码](Source/widthOfBinaryTree_1.h)
> * **DFS**：还可以用DFS，但需要额外的map结构记录每一层最左边节点的位置。

## 92. 反转链表II
> * [题目链接](https://leetcode-cn.com/problems/reverse-linked-list-ii/)
> * **一次遍历**：记录第m个节点的前驱结点和第n个结点，然后将其反战，连接原来链表即可。[我的代码](Source/reverseBetween.h)。**该方法不是严格的一次遍历**。[另一个版本代码](Source/reverseBetween.h)。注意细节
> * **递归解法，必看**：[链接](https://leetcode-cn.com/problems/reverse-linked-list-ii/solution/bu-bu-chai-jie-ru-he-di-gui-di-fan-zhuan-lian-biao/)

## 402. 移掉K位数字
> * [题目链接](https://leetcode-cn.com/problems/remove-k-digits/)
> * **单调栈，其实是贪心思想**：根据下一个排序可以，如果一个数字序列是递增的，那么它就是最小的。所以利用单调递增栈，尽可能使剩下数字序列单调递增。**注意书写细节**。[我的代码](Source/removeKdigits.h)。[别人的代码](Source/removeKdigits_2.h)。[思路讲解](https://leetcode-cn.com/problems/remove-k-digits/solution/yi-zhao-chi-bian-li-kou-si-dao-ti-ma-ma-zai-ye-b-5/)

## 6. Z字变换
> * [题目链接](https://leetcode-cn.com/problems/zigzag-conversion/)
> * **按行放置，然后组合到一起**：用一个变量`cur`表示当前应该往第一行放置；再用一个变量，表示当前行数应该增加还是减少；顺次遍历字符串即可。时间和空间复杂度均为字符串的长度。[我的代码](Source/convert.h)
> * 另一种方法就是找规律，同一行中，下一个元素应该是谁，然后直接放置到结果数组中。[官方链接](https://leetcode-cn.com/problems/zigzag-conversion/solution/z-zi-xing-bian-huan-by-leetcode/)
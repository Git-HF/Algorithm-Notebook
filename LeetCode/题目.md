...
> * LRU的本质就是一个队列，将新来的和刚刚访问过的页面放在队列尾部；当页面不够时，从头部删除页面。由于有大量的删除，所以利用链表来实现这个队列，保存队列的头部和尾部。为了可以以**O(1)**的时间复杂度访问元素，利用hash表进行查找。[我的代码](Source/LRUCache.h)。在删除节点时，注意更新`tail`，当删除的节点就是`tail`时，需要将`tail`向前移动。
